...
template <typename T> int getWinner(std::vector<TournamentTreeNode<T>*> tree, int idx) {
...
template <typename T> std::vector<TournamentTreeNode<T>*> buildTournamentTree(std::vector<T>& arr) {
    int n = arr.size();
...
template <typename T> void updateTournamentTree(std::vector<TournamentTreeNode<T>*> tree, int idx) {
    while (idx >= 0) {
...
template <typename T> void tournamentSort(std::vector<T>& arr) {
    int n = arr.size();
    auto tree = buildTournamentTree(arr);
    for (int i = 0; i < n; i++) {
        int w = tree[0]->ref;
        arr[i] = tree[w]->data;
        // use MAX to remove current winner, and rematch along the winning path
        tree[w]->data = std::numeric_limits<T>::max();
